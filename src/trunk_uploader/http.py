...
def classify(response: requests.Response | None = None, error: Exception | None = None) -> ResponseResult:
    if error is not None:
        return ResponseResult(False, True, None, type(error).__name__ + ": network failure")
    ...
    return ResponseResult(False, retryable, response.status_code, f"HTTP {response.status_code}")
