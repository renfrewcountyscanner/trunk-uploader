...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command in {"upload", "dry-run"}:
        try:
            call = _call(args); profile = select_profile(call, settings, args.profile); matches = settings.matching(profile, call.talkgroup) if call.talkgroup_known else []
        except ValueError as exc: print(str(exc), file=sys.stderr); return 2
        if args.command == "dry-run":
            selected = {(d.type, d.name) for d in matches}
            print(json.dumps({"fingerprint": call.fingerprint, "profile": profile, "audio": str(call.audio_path), "metadata": str(call.json_path), "talkgroup": call.talkgroup, "talkgroup_tag": call.talkgroup_tag, "talkgroup_description": call.talkgroup_description, "talkgroup_known": call.talkgroup_known, "start_time": call.start_time, "frequency": call.frequency, "audio_type": call.audio_type, "encrypted": call.encrypted, "system": call.system_short_name, "sources": call.sources, "aliases": call.aliases, "patches": call.patches, "emergency": call.emergency, "destinations": [{"type": d.type, "name": d.name, "url": d.url, "system_id": d.system_id, "talkgroups": d.talkgroups.raw, "rule_matched": d.matches(call.talkgroup), "selected": (d.type, d.name) in selected, "skip_reason": ("talkgroup_not_in_file" if not call.talkgroup_known else ("disabled_or_method_disabled" if (d.type, d.name) not in selected and d.enabled else ("encrypted_rdio" if d.type == "rdio" and call.encrypted else "talkgroup_filter" if not d.matches(call.talkgroup) else "not_selected")))} for d in settings.destinations if d.profile == profile]}, indent=2, default=str))
            return 0
